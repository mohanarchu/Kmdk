...
import com.video.aashi.kmdk.BuildConfig;
...
public class CameraUtils {
    ...
    public static File getOutputMediaFile(int type) {
        ...
        File mediaStorageDir = new File(
                ...
                Complaints.GALLERY_DIRECTORY_NAME);
 
        // Create the storage directory if it does not exist
        if (!mediaStorageDir.exists()) {
            if (!mediaStorageDir.mkdirs()) {
                Log.e(Complaints.GALLERY_DIRECTORY_NAME, "Oops! Failed create "
                        + Complaints.GALLERY_DIRECTORY_NAME + " directory");
                return null;
            }
        }
        ...
        File mediaFile;
        if (type == Complaints.MEDIA_TYPE_IMAGE) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "IMG_" + timeStamp + "." + Complaints.IMAGE_EXTENSION);
        } else if (type == Complaints.MEDIA_TYPE_VIDEO) {
            mediaFile = new File(mediaStorageDir.getPath() + File.separator
                    + "VID_" + timeStamp + "." + Complaints.VIDEO_EXTENSION);
        } else {
            return null;
        }
